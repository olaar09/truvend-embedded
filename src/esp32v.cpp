...
void resetMemory(){
    pzems1.resetEnergy();
    EEPROM.put(unit_addr, float(0.02));
    EEPROM.commit();
}
...
void initializeMemory() {
  ...
  if (isnan(availableUnit)) {
    resetMemory();
    EEPROM.get(unit_addr, availableUnit);   // reload after reset
    EEPROM.get(nonceAddr, lastNonce);
  }


  EEPROM.get(timeAddr, timeSeconds);
if (timeSeconds < 0 || isnan(timeSeconds)) {
    timeSeconds = 0;
}
...
float handleData(String request) {
  ...
  Serial.print("Second: ");  Serial.println(secondsStr);
...
void debugReadings() {
  ...
  // First valid run: immediately display VOLT and set timer
...
if (millis() - lastUpdate >= 1000) {   // 1 second passed
    lastUpdate = millis();
